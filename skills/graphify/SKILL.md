...
trigger: /graphify
parent: sypnose/registry
role: motor de grafo de /registry — convierte codigo/APIs/docs en grafo navegable y Mermaid
---

> **Componente de `/registry` (dentro de `/sypnose`).** `/registry` mantiene el
> inventario vivo de APIs y composicion del sistema; `/graphify` es su motor: construye
> el grafo de codigo/funciones/endpoints/servidores y lo exporta (HTML, Mermaid, SVG,
> Neo4j). Invocable suelto, pero su lugar canonico es `/sypnose` → `/registry` → grafo.
...
/graphify <path> --mermaid                            # export graphify-out/graph.mmd (Mermaid, lo leen humanos y agentes)
...
### Step 7a - Mermaid export (only if --mermaid flag)

**If `--mermaid`** — genera `graphify-out/graph.mmd` desde `graph.json`. Es la salida que
piden humanos y agentes para ver la composicion de cualquier funcion o servidor de un vistazo.

Reglas de generacion (para que SIEMPRE renderice y sea legible):
- Cabecera: `graph LR` (o `graph TD` si el grafo es mas profundo que ancho).
- Limite de nodos: default 80. Si hay mas, colapsar por comunidad (un nodo `subgraph` por
  comunidad detectada) y expandir solo la comunidad consultada. Flag `--mermaid-max N` ajusta.
- ID de nodo: sanitizar a `[A-Za-z0-9_]` (Mermaid rompe con `/`, `.`, `-`, espacios y `:`).
  Guardar el label real entre comillas: `n_getClienteFicha["getClienteFicha()"]`.
- Tipo de arista segun relacion del grafo:
  - calls/imports → `-->`
  - reads/writes (datos) → `-.->|reads|` / `-.->|writes|`
  - INFERRED (no confirmado) → `-..->` con label `inferred`
...
lines = ["graph LR"]
# nodos agrupados por comunidad
for comm, nodes in group_by_community(g["nodes"]):
    lines.append(f'  subgraph "Comunidad {comm}"')
    for nd in nodes[:MAX]:
        shape = '(["%s"])' % nd["label"] if nd["type"]=="api" else '["%s"]' % nd["label"]
        lines.append(f'    {nid(nd["id"])}{shape}')
    lines.append("  end")
# aristas
EDGE = {"calls":"-->","imports":"-->","reads":'-.->|reads|',
        "writes":'-.->|writes|',"inferred":"-..->"}
for e in g["edges"]:
    arr = EDGE.get(e["rel"], "-->")
    lines.append(f'  {nid(e["src"])} {arr} {nid(e["dst"])}')
open("graphify-out/graph.mmd","w").write("\n".join(lines))
```

Validacion obligatoria tras generar: el archivo debe empezar por `graph ` y parsear sin error
(probar en `mermaid.live` o con `mmdc -i graph.mmd -o /dev/null` si mermaid-cli esta instalado).
